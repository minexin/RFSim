...
int main() {
    ...
    const NoiseCorrelation c{
        SMatrix{2, {2. * kt, -3. * std::conj(optimum) * kt, -3. * optimum * kt, 1.39 * kt}}};
    ...
    for (const Complex gamma : {Complex{}, Complex{0.4, -0.1}, Complex{-0.3, 0.7}}) {
        const double reconstructed =
            std::pow(10., parameters.minimum_noise_figure_db / 10.) +
            4 * parameters.noise_resistance_ohms / 50. *
                std::norm(gamma - parameters.optimum_source_reflection) /
    ...
    near(passive.minimum_noise_figure_db, 10 * std::log10(4.));
    near(passive.optimum_source_reflection, 0.);
    ...
    rejects<std::domain_error>([&] {
        extract_noise_parameters(unity, {SMatrix{2, {kt, kt, kt, kt}}});
    });
    rejects<std::invalid_argument>([&] {
        extract_noise_parameters(unity, c, 0.);
    });
}
