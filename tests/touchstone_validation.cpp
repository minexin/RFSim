...
int main(int argc, char** argv) {
    ...
    auto write = [&](const std::string& data) { std::ofstream out(scratch); out << data; };
    write("#\n1 0.5 90\n");
    ...
    for (const auto& data : {
        ...
        "# Hz S DB R 50\n1 99999 0\n", "# Hz S RI R 50\n#\n1 0 0\n"}) {
        write(data);
        rejects<std::runtime_error>([&] { read_touchstone(scratch); });
    }
    rejects<std::runtime_error>([] { read_touchstone("invalid.s2pBAD"); });
    std::remove(scratch);
}
