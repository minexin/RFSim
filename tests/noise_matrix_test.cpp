...
int main() {
    ...
    near(pad(0, 0) / kt, 0.75);
    near(pad(1, 1) / kt, 0.75);
    near(pad(0, 1) / kt, 0.);
    ...
    near(thermal(0, 0) / kt, 4. / 9);
    near(thermal(0, 1) / kt, -4. / 9);
    ...
    const auto transformed =
        propagate_noise(SMatrix{2, {1., 0., 0., Complex{0., -1.}}}, source).watts_per_hz;
    for (auto x : transformed.values) {
        near(x, 1.);
    }
    // Pivoting: large variance at the second port.
    const auto pivot = propagate_noise(SMatrix{2, {0., 1., 1., 0.}},
                                       NoiseCorrelation{SMatrix{2, {0., 0., 0., 4.}}})
                           .watts_per_hz;
    near(pivot(0, 0), 4.);
    near(pivot(1, 1), 0.);
    rejects<std::invalid_argument>([] {
        passive_thermal_noise(SMatrix{1, {2.}}, 0.);
    });
    rejects<std::invalid_argument>([] {
        propagate_noise(SMatrix{2, {1., 0., 0., 1.}},
                        NoiseCorrelation{SMatrix{2, {1., 2., 2., 1.}}});
    });
    rejects<std::invalid_argument>([] {
        propagate_noise(SMatrix{2, {1., 0., 0., 1.}},
                        NoiseCorrelation{SMatrix{2, {1., Complex{0., 1.}, Complex{0., 1.}, 1.}}});
    });
    rejects<std::invalid_argument>([] {
        passive_thermal_noise(SMatrix{1, {0.}}, -1.);
    });
}
