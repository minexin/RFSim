...
int main() {
    ...
    for (std::size_t i=0;i<4;++i) near(changed.values[i],thru.values[i]);
    ...
    rejects<std::invalid_argument>([&]{renormalize_s(original,0.,75.);});
    rejects<std::domain_error>([]{renormalize_s(SMatrix{1,{5.}},50.,75.);});
}
