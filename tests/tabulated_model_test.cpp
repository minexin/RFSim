...
int main(int argc,char** argv) {
    ...
    auto model=TabulatedSParameterModel::from_touchstone("filter",std::string(argv[1])+"/sweep.s2p");
    ...
    rejects<std::out_of_range>([&]{device.port(2);});
    rejects<std::out_of_range>([&]{provider.s_parameters(4e9);});
    ...
    rejects<std::invalid_argument>([&]{TabulatedSParameterModel invalid("",data);});
}
