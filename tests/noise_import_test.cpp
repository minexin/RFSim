...
int main() {
    ...
    const TwoPortNoiseParameters parameters{10*std::log10(2.),gamma,75.*3.*std::norm(1.+gamma)/4.};
    ...
    for (std::size_t i=0;i<4;++i) near(warmer.watts_per_hz.values[i]/kt,2.*noise.watts_per_hz.values[i]/kt);
    ...
    for (std::size_t i=0;i<4;++i) near(imported.watts_per_hz.values[i]/kt,thermal.watts_per_hz.values[i]/kt);
    // Imported device data enters the same assembled network solver.
    LinearNetwork network;
    network.add(pad); network.add(pad); network.connect(1,2);
    ...
    rejects<std::invalid_argument>([&]{noise_from_parameters(s,{3.,{},0.});});
    rejects<std::invalid_argument>([&]{noise_from_parameters(s,{0.,1.,1.});});
    rejects<std::invalid_argument>([&]{noise_from_parameters(s,parameters,0.);});
    rejects<std::overflow_error>([&]{noise_from_parameters(s,{1e308,{},1.});});
    const auto zero=noise_from_parameters(s,{});
    for (auto value:zero.watts_per_hz.values) near(value,0.);
}
