...
int main(int argc,char** argv) {
    ...
    rejects<std::out_of_range>([&]{interpolate_s(data,4e9);});
    near(interpolate_s(data,4e9,OutOfBand::Clamp)(1,0),0.25);
    rejects<std::runtime_error>([&]{sweep_network(FrequencyGrid{{4e9}},build);});
    auto invalid=data;
    invalid.frequencies_hz.clear();
    rejects<std::invalid_argument>([&]{interpolate_s(invalid,1e9);});
    invalid=data; invalid.frequencies_hz[1]=1e9;
    rejects<std::invalid_argument>([&]{interpolate_s(invalid,1e9);});
    invalid=data; invalid.matrices[1].values.clear();
    rejects<std::invalid_argument>([&]{interpolate_s(invalid,1e9);});
    rejects<std::invalid_argument>([&]{interpolate_s(data,std::numeric_limits<double>::quiet_NaN());});
    int calls=0;
    rejects<std::invalid_argument>([&]{sweep_network(FrequencyGrid{{2.,1.}},[&](double){++calls;return LinearNetwork{};});});
