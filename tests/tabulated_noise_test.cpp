...
int main() {
    ...
    TouchstoneData data;
    data.ports = 2;
    data.frequencies_hz = {0., 4e9};
    data.matrices = {SMatrix{2, {0., 0., 1., 0.}}, SMatrix{2, {0., 0., 1., 0.}}};
    NoiseTable table{{1e9, 3e9},
    ...
    const auto result = analyze_linear(
        FrequencyGrid{{1e9, 2e9, 3e9}},
        {0, 1},
        [&](double f) {
            LinearNetwork net;
            net.add(model.s_parameters(f));
            return net;
        },
        [&](double f, const LinearNetwork &) {
            return provider.noise_correlation(f);
        });
    for (std::size_t i = 0; i < 3; ++i) {
        near(two_port_noise_figure_db(result.scattering[i], result.noise_correlation[i]),
             10 * std::log10(2.));
    }
    ...
    rejects<std::invalid_argument>([&] {
        TabulatedNoiseModel bad("bad", data, NoiseTable{});
    });
    single.samples[0].watts_per_hz(0, 0) = -kt;
    rejects<std::invalid_argument>([&] {
        TabulatedNoiseModel bad("bad", data, single);
    });
}
