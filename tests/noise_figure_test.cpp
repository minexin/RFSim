...
int main() {
    ...
    near(two_port_noise_figure_db(pad,noise,0.5),10*std::log10(1.+0.75*(4.+0.25)/0.75));
    ...
    LinearNetwork net;
    net.add(pad); net.add(amplifier); net.connect(1,2);
    ...
    near(two_port_noise_figure_db(net.external_s({0,3}),net.external_noise({0,3},combined)),
         cascade_noise_figure_db({{-10*std::log10(4.),10*std::log10(4.)},{10.,10*std::log10(2.)}}));
    rejects<std::invalid_argument>([&]{two_port_noise_figure_db(pad,noise,1.);});
    rejects<std::invalid_argument>([&]{two_port_noise_figure_db(pad,noise,0.,0.);});
    rejects<std::domain_error>([&]{two_port_noise_figure_db(SMatrix{2,{0.,0.,0.,0.}},noise);});
    rejects<std::domain_error>([&]{two_port_noise_figure_db(SMatrix{2,{2.,0.,1.,0.}},noise,0.5);});
}
