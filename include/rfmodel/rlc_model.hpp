...
namespace rfmodel {
enum class IdealElement { Resistor, Inductor, Capacitor };
...
public:
    IdealRLCModel(std::string name, IdealElement element, LumpedConnection connection,
                  double value, double reference_ohms=50.)
        : name_(std::move(name)),element_(element),connection_(connection),value_(value),reference_(reference_ohms) {
        if (name_.empty() || !std::isfinite(value_) || value_<=0 ||
            !std::isfinite(reference_) || reference_<=0 ||
            (element_!=IdealElement::Resistor && element_!=IdealElement::Inductor && element_!=IdealElement::Capacitor) ||
            (connection_!=LumpedConnection::SeriesImpedance && connection_!=LumpedConnection::ShuntAdmittance))
            throw std::invalid_argument("invalid ideal RLC parameters");
    }
    std::string name() const override { return name_; }
    std::size_t port_count() const override { return 2; }
    PortInfo port(std::size_t i) const override {
        if (i>=2) throw std::out_of_range("RLC port");
        return {i,i==0?"in":"out",{reference_,0.}};
    }
    SMatrix s_parameters(double f) const override {
        if (!std::isfinite(f) || f<0) throw std::invalid_argument("invalid RLC frequency");
        ...
        if (f==0 && element_!=IdealElement::Resistor) {
            const bool blocked=series?(element_==IdealElement::Capacitor):(element_==IdealElement::Inductor);
        ...
        if (element_!=IdealElement::Resistor) {
            ...
            if (element_==IdealElement::Inductor) {
                log_magnitude+=log_omega; phase={0.,1.};
            } else {
                log_magnitude=-log_omega-std::log(value_)-std::log(reference_); phase={0.,-1.};
            }
        }
        if (!series) { log_magnitude=-log_magnitude; phase=std::conj(phase); }
        log_magnitude-=std::log(2.);
        Complex t,r;
        if (log_magnitude>0) {
            const double inverse=std::exp(-log_magnitude);
            t=inverse/(inverse+phase); r=sign*phase/(inverse+phase);
        } else {
            const auto q=std::exp(log_magnitude)*phase;
            t=1./(1.+q); r=sign*q/(1.+q);
        }
        return {2,{r,t,t,r}};
    }
};
