...
inline void finite_matrix(const SMatrix& m) {
    ...
    for (auto x:m.values) if (!std::isfinite(x.real()) || !std::isfinite(x.imag()))
...
inline SMatrix psd_matrix(const SMatrix& input,double scale_floor=0.) {
    ...
    for (auto x:input.values) scale=std::max(scale,std::abs(x));
    if (!std::isfinite(scale)) throw std::overflow_error("noise scale overflow");
    ...
    for (auto& x:work.values) x/=scale;
    ...
    for (std::size_t k=0;k<n;++k) {
        std::size_t pivot=k;
        for (std::size_t i=k+1;i<n;++i) if (work(i,i).real()>work(pivot,pivot).real()) pivot=i;
        ...
        for (std::size_t j=0;j<n;++j) std::swap(work(k,j),work(pivot,j));
        for (std::size_t i=0;i<n;++i) std::swap(work(i,k),work(i,pivot));
        for (std::size_t j=0;j<k;++j) std::swap(factor(k,j),factor(pivot,j));
        std::swap(order[k],order[pivot]);
        factor(k,k)=std::sqrt(work(k,k).real());
        for (std::size_t i=k+1;i<n;++i) factor(i,k)=work(i,k)/factor(k,k);
        for (std::size_t i=k+1;i<n;++i)
            for (std::size_t j=k+1;j<n;++j) work(i,j)-=factor(i,k)*std::conj(factor(j,k));
    }
    SMatrix result{n,std::vector<Complex>(n*n)};
    for (std::size_t i=0;i<n;++i)
        for (std::size_t j=0;j<n;++j) {
            Complex value=0.;
            for (std::size_t k=0;k<n;++k) value+=factor(i,k)*std::conj(factor(j,k));
...
inline NoiseCorrelation passive_thermal_noise(const SMatrix& s,double temperature_k) {
    ...
    for (std::size_t i=0;i<s.ports;++i)
        for (std::size_t j=0;j<s.ports;++j) {
            ...
            for (std::size_t k=0;k<s.ports;++k) value-=s(i,k)*std::conj(s(j,k));
            defect(i,j)=value;
        }
    // Check passivity even at zero temperature.
    auto covariance=noise_detail::psd_matrix(defect,1.);
    constexpr double boltzmann=1.380649e-23;
    for (auto& x:covariance.values) x*=boltzmann*temperature_k;
...
inline NoiseCorrelation propagate_noise(const SMatrix& transfer,const NoiseCorrelation& source) {
    ...
    if (transfer.ports!=c.ports) throw std::invalid_argument("noise transfer dimensions differ");
    const auto n=c.ports;
    SMatrix intermediate{n,std::vector<Complex>(n*n)}, result=intermediate;
    for (std::size_t i=0;i<n;++i)
        for (std::size_t j=0;j<n;++j)
            for (std::size_t k=0;k<n;++k) intermediate(i,j)+=transfer(i,k)*c(k,j);
    for (std::size_t i=0;i<n;++i)
        for (std::size_t j=0;j<n;++j)
            for (std::size_t k=0;k<n;++k) result(i,j)+=intermediate(i,k)*std::conj(transfer(j,k));
