...
public:
    MatchedTransmissionModel(std::string name, double loss_db, double delay_s=0,
                             double reference_ohms=50)
        : name_(std::move(name)),loss_db_(loss_db),delay_s_(delay_s),reference_(reference_ohms) {
        if (name_.empty() || !std::isfinite(loss_db_) || loss_db_<0 ||
            !std::isfinite(delay_s_) || delay_s_<0 || !std::isfinite(reference_) || reference_<=0)
    ...
    PortInfo port(std::size_t i) const override {
        if (i>=2) throw std::out_of_range("transmission model port");
        return {i,i==0 ? "in" : "out",{reference_,0}};
    }
    SMatrix s_parameters(double f) const override {
        if (!std::isfinite(f) || f<0) throw std::invalid_argument("invalid frequency");
        const double cycles=f*delay_s_;
        if (!std::isfinite(cycles)) throw std::overflow_error("delay phase overflow");
