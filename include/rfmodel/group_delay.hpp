...
inline std::vector<GroupDelayInterval> group_delay(const FrequencyGrid &grid,
    ...
    for (std::size_t i = 0; i < response.size(); ++i) {
        ...
        if (response[i] == Complex{}) {
            throw std::domain_error("group delay undefined at zero response");
        }
    }
    ...
    for (std::size_t i = 1; i < response.size(); ++i) {
        ...
        const double phase =
            std::remainder(std::arg(response[i]) - std::arg(response[i - 1]), 2 * pi);
        if (std::abs(phase) == pi) {
            throw std::domain_error("ambiguous half-turn phase increment");
        }
        const double delay = -(phase / (2 * pi)) / aperture;
        if (!std::isfinite(delay)) {
            throw std::overflow_error("group delay overflow");
        }
