...
#include <limits>
namespace rfmodel {
enum class LumpedConnection { SeriesImpedance, ShuntAdmittance };
...
class LumpedTwoPortModel final : public RFDeviceModel, public SParameterProvider {
    ...
    double reference_;
public:
    LumpedTwoPortModel(std::string name, LumpedConnection connection,
                       std::function<Complex(double)> law, double reference_ohms=50.)
        : name_(std::move(name)), connection_(connection), law_(std::move(law)), reference_(reference_ohms) {
    ...
    PortInfo port(std::size_t i) const override {
        if (i>=2) throw std::out_of_range("lumped model port");
    ...
    SMatrix s_parameters(double frequency_hz) const override {
        ...
        const double scale=std::max(2.,std::max(std::abs(normalized.real()),std::abs(normalized.imag())));
