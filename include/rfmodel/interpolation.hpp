...
#include <algorithm>
namespace rfmodel {
enum class OutOfBand { Reject, Clamp };
inline void validate_s_data(const TouchstoneData& d) {
    if (!d.ports || d.ports > 1024 || d.matrices.empty() || d.frequencies_hz.size()!=d.matrices.size())
...
inline SMatrix interpolate_s(const TouchstoneData& d, double f, OutOfBand policy=OutOfBand::Reject) {
    validate_s_data(d);
    if (!std::isfinite(f) || f<0) throw std::invalid_argument("invalid query frequency");
    if (f<d.frequencies_hz.front() || f>d.frequencies_hz.back()) {
        if (policy==OutOfBand::Reject) throw std::out_of_range("frequency outside model range");
    ...
    if (*it==f) return d.matrices[i];
    const double t=(f-d.frequencies_hz[i-1])/(d.frequencies_hz[i]-d.frequencies_hz[i-1]);
