...
class LinearNetwork {
    ...
    static bool finite(Complex x) {
        return std::isfinite(x.real()) && std::isfinite(x.imag());
    }

    void available(std::size_t p) const {
        if (p >= boundaries_.size()) {
            throw std::out_of_range("network port");
        }
        if (boundaries_[p].assigned) {
            throw std::invalid_argument("port already connected or terminated");
        }
    }

    void validate_external_ports(const std::vector<std::size_t> &ports) const {
        if (ports.empty()) {
            throw std::invalid_argument("no external ports");
        }
        std::vector<bool> selected(boundaries_.size(), false);
        for (auto p : ports) {
            available(p);
            if (selected[p]) {
                throw std::invalid_argument("duplicate external port");
            }
            selected[p] = true;
        }
        for (std::size_t p = 0; p < boundaries_.size(); ++p) {
            ...
            if (boundaries_[p].source != Complex{}) {
                throw std::invalid_argument(
                    "external extraction requires zero independent sources");
            }
        }
    }

public:
    double reference_impedance_ohms() const {
        return reference_;
    }
    ...
    NoiseCorrelation external_noise(const std::vector<std::size_t> &ports,
        ...
        for (std::size_t r = 0; r < n; ++r) {
            ...
            for (std::size_t p = 0; p < n; ++p) {
                ...
                if (!boundary.assigned) {
                    continue; // Matched external port.
                }
                const Complex multiplier =
                    boundary.partner == p ? boundary.reflection : Complex{1., 0.};
        ...
        for (std::size_t r = 0; r < ports.size(); ++r) {
            for (std::size_t c = 0; c < ports.size(); ++c) {
                result(r, c) = full(ports[r], ports[c]);
            }
        }
    ...
    explicit LinearNetwork(double reference_ohms = 50.0) : reference_(reference_ohms) {
        if (!std::isfinite(reference_) || reference_ <= 0) {
            throw std::invalid_argument("invalid network reference");
        }
    }

    // Returns the first global port index of this block.
    std::size_t add(const SMatrix &s, double reference_ohms = 50.0) {
        if (reference_ohms != reference_) {
            throw std::invalid_argument("reference renormalization required");
        }
        ...
        if (offset + s.ports > 1024) {
            throw std::invalid_argument("dense solver port limit exceeded");
        }
        blocks_.push_back({s, offset});
        for (std::size_t i = 0; i < s.ports; ++i) {
            boundaries_.push_back({offset + i, {}, {}, false});
        }
        return offset;
    }
    ...
    void connect(std::size_t p, std::size_t q) {
        ...
        available(q);
        if (p == q) {
            throw std::invalid_argument("cannot connect port to itself");
        }
    ...
    void terminate(std::size_t p, Complex reflection = {}, Complex source = {}) {
        available(p);
        if (!finite(reflection) || !finite(source)) {
            throw std::invalid_argument("nonfinite boundary");
        }
    ...
    NetworkWaves solve() const {
        ...
        for (const auto &b : boundaries_) {
            if (!b.assigned) {
                throw std::invalid_argument("unterminated port");
            }
        }
        ...
        for (std::size_t r = 0; r < n; ++r) {
            ...
            for (std::size_t c = 0; c < n; ++c) {
                ...
                if (!finite(matrix(r, c))) {
                    throw std::runtime_error("network coefficient overflow");
                }
        ...
        for (std::size_t k = 0; k < n; ++k) {
            ...
            for (std::size_t r = k + 1; r < n; ++r) {
                if (std::abs(matrix(r, k)) > std::abs(matrix(pivot, k))) {
                    pivot = r;
                }
            }
            if (std::abs(matrix(pivot, k)) <= threshold) {
                throw std::runtime_error("singular or ill-conditioned network");
            }
            for (std::size_t c = 0; c < n; ++c) {
        ...
        for (std::size_t r = n; r-- > 0;) {
            ...
            for (std::size_t c = r + 1; c < n; ++c) {
                value -= matrix(r, c) * result.incident[c];
            }
            result.incident[r] = value / matrix(r, r);
            if (!finite(result.incident[r])) {
                throw std::runtime_error("network solution overflow");
            }
        }
        ...
        for (std::size_t r = 0; r < n; ++r) {
            ...
            if (!finite(result.outgoing[r]) || !finite(error)) {
                throw std::runtime_error("network response overflow");
            }
