...
inline Complex input_impedance(Complex reflection, double reference_ohms = 50.0) {
    ...
    if (reflection == Complex{1, 0}) {
        throw std::domain_error("open circuit has infinite impedance");
    }
    const Complex value =
        reference_ohms * ((Complex{1, 0} + reflection) / (Complex{1, 0} - reflection));
...
inline Complex input_admittance(Complex reflection, double reference_ohms = 50.0) {
    ...
    if (reflection == Complex{-1, 0}) {
        throw std::domain_error("short circuit has infinite admittance");
    }
    const Complex value =
        ((Complex{1, 0} - reflection) / (Complex{1, 0} + reflection)) / reference_ohms;
    if (!std::isfinite(value.real()) || !std::isfinite(value.imag())) {
...
inline PortPower port_power(Complex incident, Complex outgoing) {
    ...
    if (!std::isfinite(a) || !std::isfinite(b)) {
        throw std::invalid_argument("nonfinite wave power");
    }
    return {a, b, a - b};
}
...
inline double dbm_to_watts(double dbm) {
    if (dbm == -std::numeric_limits<double>::infinity()) {
        return 0;
    }
    if (!std::isfinite(dbm)) {
        throw std::invalid_argument("invalid dBm");
    }
...
inline double power_gain_db(double output_w, double input_w) {
    if (!std::isfinite(input_w) || input_w <= 0) {
        throw std::invalid_argument("positive input power required");
    }
...
inline double return_loss_db(Complex reflection) {
    ...
    if (!std::isfinite(magnitude)) {
        throw std::invalid_argument("invalid reflection");
    }
...
inline double vswr(Complex reflection) {
    const double magnitude = std::abs(reflection);
    if (!std::isfinite(magnitude) || magnitude > 1) {
        throw std::invalid_argument("VSWR requires |Gamma|<=1");
    }
    return magnitude == 1 ? std::numeric_limits<double>::infinity()
