...
class TabulatedSParameterModel final : public RFDeviceModel, public SParameterProvider {
    ...
    OutOfBand policy_;
public:
    TabulatedSParameterModel(std::string name, TouchstoneData data,
                            OutOfBand policy=OutOfBand::Reject)
        : name_(std::move(name)), data_(std::move(data)), policy_(policy) {
        if (name_.empty()) throw std::invalid_argument("empty model name");
        validate_s_data(data_);
    }
    static TabulatedSParameterModel from_touchstone(
        std::string name, const std::string& path, OutOfBand policy=OutOfBand::Reject) {
        return TabulatedSParameterModel(std::move(name),read_touchstone(path),policy);
    }
    std::string name() const override { return name_; }
    std::size_t port_count() const override { return data_.ports; }
    PortInfo port(std::size_t index) const override {
        if (index>=data_.ports) throw std::out_of_range("model port index");
    ...
    double minimum_frequency_hz() const { return data_.frequencies_hz.front(); }
    double maximum_frequency_hz() const { return data_.frequencies_hz.back(); }
};
}
