...
`external_s({port0,port1,...})` 在每个选定外端口施加单位入射波，其余外端口匹配，逐列得到等效 S 矩阵。参数中的端口顺序决定结果矩阵的行列顺序。方法保持原网络不变。

选定端口必须尚未连接或终端化；其余端口必须已有连接或无源激励边界（独立 source=0，反射系数可非零）。重复端口、缺失内部边界、独立激励均拒绝。参考阻抗沿用网络统一正实数参考。

当前逐列复制网络并重新求解，尚未复用矩阵分解，适合功能验证，性能仍待改进。测试采用非互易两级网络的四个解析 S 参数（含内部多次反射）、端口逆序、反射终端及错误输入。
