...
MSVC x64 Debug 和 Release 均构建成功，CTest 各 6/6 通过。测试使用始终有效的检查函数，Release 不会因 NDEBUG 而跳过断言。
...
- Touchstone 已验证基础 RI/MA/DB 转换和单位换算，完整限制见 touchstone.md；插值已纳入扫描回归；噪声尚未纳入测试。
